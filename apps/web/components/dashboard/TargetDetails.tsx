...
export function TargetDetails({ id, name, url, onClose }: TargetDetailsProps) {
  ...
  const runTest = async () => {
    try {
      ...
      const triggerRes = await axiosInstance.post("/audits/trigger", {
        targetId: id,
      });
        ...
        try {
          ...
          if (jobData.status === "completed") {
            ...
            const resultRes = await axiosInstance.get(
              `/audits/audit-results?targetId=${id}&limit=1`,
            );
            ...
            setLogs((prev) => [
              ...prev,
              `! Audit Job Failed: ${jobData.errorLog || "Unknown error"}`,
            ]);
            setStatus("failed");
          }
        } catch (pollErr) {
      ...
      clearTimers();
      setLogs([
        `! Failed to initiate audit: ${err.response?.data?.message || err.message}`,
      ]);
      setStatus("failed");
    }
  };
  ...
  const getPct = (val: number) =>
    auditResult
      ? ((val / parseFloat(totalHandshakeTime)) * 100).toFixed(1) + "%"
      : "0%";
  ...
  const distributionData = auditResult
    ? [
        ...
        {
          range: "50-100",
          density: auditResult.p50 >= 50 && auditResult.p50 < 100 ? 45 : 10,
          p95: 0,
        },
        {
          range: "100-150",
          density: auditResult.p50 >= 100 && auditResult.p50 < 150 ? 50 : 15,
          p95: auditResult.p95 > 100 ? 5 : 0,
        },
        {
          range: "150-200",
          density: 20,
          p95: auditResult.p95 >= 150 && auditResult.p95 < 200 ? 10 : 2,
        },
        {
          range: "200-250",
          density: 8,
          p95: auditResult.p95 >= 200 && auditResult.p95 < 250 ? 8 : 1,
        },
          ...
          {status === "idle" ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 text-sm">
              ...
              <p className="text-neutral-400 mb-1">
                Awaiting probe. Enter a URL and trigger a{" "}
                <span className="text-cyan-400 font-medium">burst test</span>.
              </p>
        ...
        <div className="flex flex-col gap-6 mt-4 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-mono text-neutral-500 tracking-tight">
                ...
                <span className="text-neutral-900">
                  aud_{auditResult.id?.slice(0, 8)}
                </span>{" "}
                · 100 req · 100 ok · 0 failed ·{" "}
                {auditResult.totalTime.toFixed(2)}ms
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                label: "P50 Latency",
                value: auditResult.p50.toFixed(2),
                unit: "ms",
                color: "text-blue-600",
                icon: Activity,
              },
              {
                label: "P95 Latency",
                value: auditResult.p95.toFixed(2),
                unit: "ms",
              ...
              {
                label: "P99 Latency",
                value: auditResult.p99.toFixed(2),
                unit: "ms",
              ...
              {
                label: "Std. Deviation",
                value: auditResult.stdDev.toFixed(2),
                unit: "ms",
              ...
              <div
                key={i}
                className="flex flex-col gap-3 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm relative overflow-hidden"
              >
            ...
            <div className="flex flex-col gap-6 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
              ...
              <div className="flex h-3 w-full rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: getPct(auditResult.dnsTime) }}
                ></div>
                <div
                  className="bg-purple-400 h-full"
                  style={{ width: getPct(auditResult.tcpTime) }}
                ></div>
                <div
                  className="bg-orange-400 h-full"
                  style={{ width: getPct(auditResult.tlsTime) }}
                ></div>
                <div
                  className="bg-green-400 h-full"
                  style={{ width: getPct(auditResult.ttfb) }}
                ></div>
              </div>
              ...
              <div className="flex flex-col gap-3 mt-2">
                {[
                  {
                    label: "DNS",
                    value: auditResult.dnsTime.toFixed(2) + "ms",
                    pct: getPct(auditResult.dnsTime),
                    color: "bg-blue-400",
                  },
                  {
                    label: "TCP",
                    value: auditResult.tcpTime.toFixed(2) + "ms",
                    pct: getPct(auditResult.tcpTime),
                    color: "bg-purple-400",
                  },
                  {
                    label: "TLS",
                    value: auditResult.tlsTime.toFixed(2) + "ms",
                    pct: getPct(auditResult.tlsTime),
                    color: "bg-orange-400",
                  },
                  {
                    label: "Processing (TTFB)",
                    value: auditResult.ttfb.toFixed(2) + "ms",
                    pct: getPct(auditResult.ttfb),
                    color: "bg-green-400",
                  },
                ].map((item, i) => (
                  <div
                    ...
                    <Bar dataKey="p95" radius={[2, 2, 0, 0]} maxBarSize={40}>
                      {distributionData.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            entry.p95 > 0 ? "url(#colorOrange)" : "transparent"
                          }
                        />
                      ))}
                    </Bar>
